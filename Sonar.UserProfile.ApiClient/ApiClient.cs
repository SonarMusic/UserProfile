...
namespace Sonar.UserProfile.ApiClient
{
    ...
    public partial interface IUserApiClient
    {
        ...
        /// Delete token.
        /// </summary>
        /// <param name="token">DTO which contains token (ID of token to be precise).</param>
        /// <returns>Success</returns>
        /// <exception cref="ApiException">A server side error occurred.</exception>
            ...
            else
            {
                try
                {
                    using (var responseStream = await response.Content.ReadAsStreamAsync().ConfigureAwait(false))
                    {
                        ...
                        return new ObjectResponseResult<T>(typedBody, typedBody.ToString());
                    }
                }
                catch (System.Text.Json.JsonException exception)
                {
                    var message = "Could not deserialize the response body stream as " + typeof(T).FullName + ".";
                    throw new ApiException(message, (int)response.StatusCode, message, headers, exception);
                }
            }
        }
